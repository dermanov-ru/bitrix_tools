...
{
    /**
     * ћасштабирует фото, сохран€ет копию файла и возвращает путь к нему
     * либо возвращает ссылку на картинку-заглушку
     *
     * ---
     *
     * ¬од€ной знак - если существует файл /upload/watermark/watermark_original.png - он будет
     * смасштабирован под фото и нанесен на всю поверхность с небольшим отступом от кра€.
     * watermark_original.png - должен быть большого размера, чтобы не тер€лось качество.
     *
     * @param $imgId
     * @param $width int
     * @param $height int ≈сли не задано, будет равно ширине
     *
     * @throws Exception File dimensions can not be a null
     *
     *
     * @return string ѕуть к измененному файлу
     */
    public static function getResizedImgOrPlaceholder( $imgId, $width, $height = 0, $proportional = false){
         ...
         * 2) создаем вод€ной знак под этот размер фото (он должен быть чуть меньше самого фото)
         * 3) формируем фильтр дл€ наложени€ знака
         * */
        ...
        /* если файл по каким-то причинам не создалс€ */
